...
train_set=$KALDI_DATA_LOCATION_TOKENIZED/train_cleaned
gmm=$KALDI_MODEL_LOCATION_TOKENIZED/tri4_ali  # the gmm for the target data
num_threads_ubm=8
...
tree_dir=$KALDI_MODEL_LOCATION_TOKENIZED/chain${nnet3_affix}/tree_bi${tree_affix}
lat_dir=$KALDI_MODEL_LOCATION_TOKENIZED/chain${nnet3_affix}/${gmm}_${train_set}_sp_lats
dir=$KALDI_MODEL_LOCATION_TOKENIZED/chain${nnet3_affix}/tdnn${tdnn_affix}_sp
train_data_dir=$KALDI_DATA_LOCATION_TOKENIZED/${train_set}_sp_hires
lores_train_data_dir=$KALDI_DATA_LOCATION_TOKENIZED/${train_set}_sp
train_ivector_dir=$KALDI_MODEL_LOCATION_TOKENIZED/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
...
if [ $stage -le 14 ]; then
  ...
  if [ -d $KALDI_DATA_LOCATION_TOKENIZED/lang_chain ]; then
    if [ $KALDI_DATA_LOCATION_TOKENIZED/lang_chain/L.fst -nt $KALDI_DATA_LOCATION_TOKENIZED/lang/L.fst ]; then
    ...
    cp -r $KALDI_DATA_LOCATION_TOKENIZED/lang $KALDI_DATA_LOCATION_TOKENIZED/lang_chain
    silphonelist=$(cat $KALDI_DATA_LOCATION_TOKENIZED/lang_chain/phones/silence.csl) || exit 1;
    nonsilphonelist=$(cat $KALDI_DATA_LOCATION_TOKENIZED/lang_chain/phones/nonsilence.csl) || exit 1;
    # Use our special topology... note that later on may have to tune this
    # topology.
    $KALDI_TEDLIUM/steps/nnet3/chain/gen_topo.py $nonsilphonelist $silphonelist >$KALDI_DATA_LOCATION_TOKENIZED/lang_chain/topo
  fi
fi
...
if [ $stage -le 15 ]; then
  ...
  $KALDI_TEDLIUM/steps/align_fmllr_lats.sh --nj 100 --cmd "$train_cmd" ${lores_train_data_dir} \
    $KALDI_DATA_LOCATION_TOKENIZED/lang $gmm_dir $lat_dir
...
if [ $stage -le 16 ]; then
  ...
  $KALDI_TEDLIUM/steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
      --context-opts "--context-width=2 --central-position=1" \
      --cmd "$train_cmd" 4000 ${lores_train_data_dir} $KALDI_DATA_LOCATION_TOKENIZED/lang_chain $ali_dir $tree_dir
fi
...
EOF
  $KALDI_TEDLIUM/steps/nnet3/xconfig_to_configs.py --xconfig-file $dir/configs/network.xconfig --config-dir $dir/configs/
...
if [ $stage -le 18 ]; then
  ...
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $dir/egs/storage ]; then
    $KALDI_TEDLIUM/utils/create_split_dir.pl \
...
if [ $stage -le 19 ]; then
  ...
  # the lang directory.
  $KALDI_TEDLIUM/utils/mkgraph.sh --self-loop-scale 1.0 $KALDI_DATA_LOCATION_TOKENIZED/lang $dir $dir/graph
fi
...
if [ $stage -le 20 ]; then
  ...
  for dset in dev test; do
      ...
      $KALDI_TEDLIUM/steps/nnet3/decode.sh --num-threads 4 --nj $decode_nj --cmd "$decode_cmd" \
          ...
          --online-ivector-dir $KALDI_MODEL_LOCATION_TOKENIZED/nnet3${nnet3_affix}/ivectors_${dset}_hires \
          --scoring-opts "--min-lmwt 5 " \
         $dir/graph $KALDI_DATA_LOCATION_TOKENIZED/${dset}_hires $dir/decode_${dset} || exit 1;
      $KALDI_TEDLIUM/steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" $KALDI_DATA_LOCATION_TOKENIZED/lang $KALDI_DATA_LOCATION_TOKENIZED/lang_rescore \
        $KALDI_DATA_LOCATION_TOKENIZED/${dset}_hires ${dir}/decode_${dset} ${dir}/decode_${dset}_rescore || exit 1
    ) || touch $dir/.error &
  done
  wait
  if [ -f $dir/.error ]; then
