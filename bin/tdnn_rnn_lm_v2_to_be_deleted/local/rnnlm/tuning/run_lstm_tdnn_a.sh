...
dir=$KALDI_MODEL_LOCATION_TOKENIZED/rnnlm_lstm_tdnn_a
embedding_dim=800
lstm_rpd=200
lstm_nrpd=200
stage=-10
...
text_from_audio=$KALDI_DATA_LOCATION_TOKENIZED/train/text
wordlist=$KALDI_DATA_LOCATION_TOKENIZED/lang_chain/words.txt
dev_sents=10000
text_dir=$KALDI_DATA_LOCATION_TOKENIZED/rnnlm/text
mkdir -p $dir/config
set -e
...
if [ $stage -le 0 ]; then
  ...
  cat $KALDI_DATA_LOCATION_TOKENIZED/rnnlm/full_lm_data.shuffled | head -n $dev_sents> $text_dir/dev.txt
  cat $KALDI_DATA_LOCATION_TOKENIZED/rnnlm/full_lm_data.shuffled | tail -n +$[$dev_sents+1] > $text_dir/ted.txt
...
EOF
  ...
  # choose features
  $KALDI_TEDLIUM/rnnlm/choose_features.py --unigram-probs=$dir/config/unigram_probs.txt \
...
EOF
  $KALDI_TEDLIUM/rnnlm/validate_config_dir.sh $text_dir $dir/config
fi
...
if [ $stage -le 2 ]; then
  ...
  $KALDI_TEDLIUM/rnnlm/prepare_rnnlm_dir.sh --unigram-factor 200.0 \
                             $text_dir $dir/config $dir
fi
echo "rnnlm dir done"

if [ $stage -le 3 ]; then
  $KALDI_TEDLIUM/rnnlm/train_rnnlm.sh --num-jobs-initial 1 --num-jobs-final 1 \
