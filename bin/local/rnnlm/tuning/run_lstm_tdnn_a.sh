...
dir=$KALDI_MODEL_LOCATION/rnnlm_lstm_tdnn_a
embedding_dim=800
lstm_rpd=200
lstm_nrpd=200
stage=-10
...
text_from_audio=$KALDI_DATA_LOCATION/train/text
wordlist=$KALDI_DATA_LOCATION/lang_chain/words.txt
dev_sents=10000
text_dir=$KALDI_DATA_LOCATION/rnnlm/text
mkdir -p $dir/config
set -e
...
if [ $stage -le 0 ]; then
  ...
  cat $text_from_audio | cut -d ' ' -f2- | cat $text_dir/train.txt |\
    shuf > $KALDI_DATA_LOCATION/rnnlm/full_lm_data.shuffled
  # create dev and train sets based on audio and LM data
  cat $KALDI_DATA_LOCATION/rnnlm/full_lm_data.shuffled | head -n $dev_sents> $text_dir/dev.txt
  cat $KALDI_DATA_LOCATION/rnnlm/full_lm_data.shuffled | tail -n +$[$dev_sents+1] > $text_dir/ted.txt
