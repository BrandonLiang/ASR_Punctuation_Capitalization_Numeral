...
tree_dir=$KALDI_MODEL_LOCATION/chain${nnet3_affix}/tree_bi${tree_affix}
lat_dir=$KALDI_MODEL_LOCATION/chain${nnet3_affix}/${gmm}_${train_set}_sp_lats
dir=$KALDI_MODEL_LOCATION/chain${nnet3_affix}/tdnn${tdnn_affix}_sp
train_data_dir=$KALDI_DATA_LOCATION/${train_set}_sp_hires
lores_train_data_dir=$KALDI_DATA_LOCATION/${train_set}_sp
train_ivector_dir=$KALDI_MODEL_LOCATION/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
...
if [ $stage -le 14 ]; then
  ...
  if [ -d $KALDI_DATA_LOCATION/lang_chain ]; then
    if [ $KALDI_DATA_LOCATION/lang_chain/L.fst -nt $KALDI_DATA_LOCATION/lang/L.fst ]; then
    ...
    cp -r $KALDI_DATA_LOCATION/lang $KALDI_DATA_LOCATION/lang_chain
    silphonelist=$(cat $KALDI_DATA_LOCATION/lang_chain/phones/silence.csl) || exit 1;
    nonsilphonelist=$(cat $KALDI_DATA_LOCATION/lang_chain/phones/nonsilence.csl) || exit 1;
    # Use our special topology... note that later on may have to tune this
    # topology.
    $KALDI_TEDLIUM/steps/nnet3/chain/gen_topo.py $nonsilphonelist $silphonelist >$KALDI_DATA_LOCATION/lang_chain/topo
  fi
fi
...
if [ $stage -le 15 ]; then
  ...
  $KALDI_TEDLIUM/steps/align_fmllr_lats.sh --nj 100 --cmd "$train_cmd" ${lores_train_data_dir} \
    $KALDI_DATA_LOCATION/lang $gmm_dir $lat_dir
...
if [ $stage -le 16 ]; then
  ...
  $KALDI_TEDLIUM/steps/nnet3/chain/build_tree.sh --frame-subsampling-factor 3 \
      ...
      --cmd "$train_cmd" 4000 ${lores_train_data_dir} $KALDI_DATA_LOCATION/lang_chain $ali_dir $tree_dir
fi
