...
$SCRIPT_DIR/steps/online/nnet3/prepare_online_decoding.sh\
  ...
  $KALDI_DATA_LOCATION/lang_chain \
  $KALDI_MODEL_LOCATION/nnet3/extractor \
  $KALDI_MODEL_LOCATION/rnnlm_lstm_tdnn_a_averaged \
  $KALDI_MODEL_LOCATION/rnnlm_lstm_tdnn_a_averaged_online

# 2. make HCLG.fst graph
$SCRIPT_DIR/utils/mkgraph.sh \
  --self-loop-scale 1.0 \
  $KALDI_DATA_LOCATION/test \
  $KALDI_MODEL_LOCATION/rnnlm_lstm_tdnn_a_averaged_online \
  $KALDI_MODEL_LOCATION/rnnlm_lstm_tdnn_a_averaged_online/graph
...
$SCRIPT_DIR/steps/online/nnet3/decode.sh \
  ...
  $KALDI_MODEL_LOCATION/rnnlm_lstm_tdnn_a_averaged_online/graph \
  $KALDI_DATA_LOCATION/test \
  $KALDI_MODEL_LOCATION/rnnlm_lstm_tdnn_a_averaged_online/decode_test

# 4. decode the lattices
mkdir -p $INFERENCE
$KALDI_ROOT/src/latbin/lattice-best-path ark:'gunzip -c $KALDI_MODEL_LOCATION/rnnlm_lstm_tdnn_a_averaged_online/decode_test/lat.*.gz |' 'ark,t:|int2sym.pl -f 2- $KALDI_MODEL_LOCATION/rnnlm_lstm_tdnn_a_averaged_online/graph/words.txt > $INFERENCE/decoded_text.txt'
